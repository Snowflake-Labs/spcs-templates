...
#   python3 generateJWT.py --account=<account_identifier> --user=<username> --private_key=<private_key>
...
class JWTGenerator(object):
    ...
    def __init__(self, account: Text, user: Text, private_key_string: Text,
        ...
        :param private_key_string: Private key string used for signing the JWTs.
        ...
        try:
            ...
            self.private_key = load_pem_private_key(private_key_string, None, default_backend())
        except TypeError:
            # If that fails, provide the passphrase returned from get_private_key_passphrase().
            self.private_key = load_pem_private_key(private_key_string, get_private_key_passphrase().encode(), default_backend())
...
def main():
    ...
    cli_parser.add_argument('--private_key_string', required=True, help='Private key used for signing the JWT.')
    ...
    token = JWTGenerator(args.account, args.user, args.private_key_string, timedelta(minutes=args.renewal_delay)).get_token()
    print('JWT:')
    print(token)
